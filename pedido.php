		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td height="28" bgcolor="#CECFCE" class="over">
                <form action="res_pes_prod.php?cli=<?=$_SESSION['controleCli2']?>" method="POST" name="Codigo" class="Estilo10">
                    <input type="text2" name="ref" id='ref'>
                </form>            </td>
            <td bgcolor="#CECFCE" class="over">
                <form action="res_pes_prod.php?cli=<?=$_SESSION['controleCli2']?>" method="POST" name="Descricao" class="Estilo10">
...
if (isset ($_GET['acao'])) {
    ...
    $vl_cambio_guarani = $linha_cambio['vl_cambio'];
				...
				$sql_cont = "SELECT COUNT(*) AS n_prod FROM carrinho WHERE codigo_prod = '$id_prod' AND status = 'A' AND controle = '".$_SESSION['controleCli2']."' AND sessao = '".session_id()."' ";
				...
				/*	 $sql_cambio = "SELECT max(cm.id) as cambio_id, cm.vl_cambio, nm_moeda FROM cambio_moeda cm, moeda WHERE cm.moeda_id = 3 AND moeda.id = 3 GROUP BY cm.id DESC limit 0,1 ";
					...
					*/
					...
					$sql_add = "INSERT INTO carrinho
								(Codigo, descricao, prvenda, qtd_produto, sessao, controle, data, status, codigo_prod)
								VALUES
								('".mysql_escape_string($reg_prod['Codigo'])."', '".mysql_escape_string($reg_prod['Descricao'])."',
								'$novovalor', '$qtd_ini', '".session_id()."',".$_SESSION['controleCli2'].", NOW(),'A', '".$reg_prod['id']."' )";
			...
			if (is_array($_POST['qtd'])) {
				foreach ($_POST['qtd'] as $Codigo => $qtd) {
						$sql_alt = "UPDATE carrinho SET qtd_produto = '$qtd' WHERE
									 codigo_prod = '$Codigo' AND sessao = '".session_id()."'";
		...
		if (($_POST['prvenda'])) {
			if (is_array($_POST['prvenda'])) {
				foreach ($_POST['prvenda'] as $Codigo => $prc) {
					...
					$sql_prod = "SELECT id,Codigo,pr_min FROM produtos WHERE id = '$Codigo'";
				    $exe_prod = mysql_query($sql_prod, $base) or die (mysql_error());
					$reg_prod = mysql_fetch_array($exe_prod, MYSQL_ASSOC );
					$prmin = $reg_prod['pr_min'];
					if($prc >= $prmin){
	...
	$conteudo.="No se puede vender abajo del precio minimo";
						}
...
<?
			if(!$menu){
				...
				$sql_lista = "SELECT * FROM carrinho WHERE controle = '".$_SESSION['controleCli2']."' 
				AND status = 'A' AND data ='".date("Y-m-d")."' AND sessao = '".session_id()."'order by id desc";
				...
				if ($num_lista > 0) {
					$total_carrinho = 0;
						...
						?>
						 <tr>
						  <td width="7%">
                            <a href="pedido.php?acao=del&id=<?=$reg_lista['id']?>" class="Estilo2"><img src="images/delete.gif" width="12" border="0"/></a>                          </td>
						  ...
						  <td width="13%"><input name="prvenda[<?=$reg_lista['codigo_prod']?>]" type="text" id="prvenda_<?=$reg_lista['codigo_prod']?>" onKeyPress="nextFocus(event,'frmItens','prvenda_<?=$reg_lista["codigo_prod"]?>')" value="<?=$reg_lista['prvenda']?>" size="9" /></td>
						  <td width="10%"><input name="qtd[<?=$reg_lista['codigo_prod']?>]" type="text" id="qtd_<?=$reg_lista['codigo_prod']?>" onKeyPress="nextFocus(event,'frmItens','qtd_<?=$reg_lista["codigo_prod"]?>')" value="<?=$reg_lista['qtd_produto']?>" size="3" /></td>
...
<table width="100%" border="0">
 ...
 <div class="Estilo4" ><? echo $conteudo; ?> </div>
    <td width="16%" height="40" rowspan="3" class="over Estilo2"><?
        ...
        <? } ?></td>
    <td width="14%" rowspan="3" class="over Estilo2"><? if($tens_maximo2 == 15){ echo "<strong>Limite maximo de itens atingido</strong>";}?>&nbsp;</td>
    <td width="17%" rowspan="3" class="over">
      <label>
    ...
    <td width="16%" rowspan="3" class="over"><input name="button" type="button" onClick="window.close()" value="Abandonar" /></td>
  ...
  <form action="pedido.php?acao=desconto" method="post">
	<?php
	echo 	$_POST['acao'] ;		
	if($_GET['acao'] == 'desconto'){
	echo	$_SESSION['desconto'] = $_POST['vldesconto'];
		}
	?>
  ...
  <tr>
    <td width="25%" class="over Estilo2"><div align="right">DESCUENTO&nbsp;</div></td>
	
    <td class="over"><label>
	
      <input type="text" size="10" name="vldesconto" id="vldesconto" value="<?=$_POST['vldesconto']?>">
    </label> </td>  </tr>
	</form>
  <tr>
    ...
    <td width="12%" class="over"><span class="Estilo2"><?
					...
					$totalGeral = $total_carrinho + $valor_frete - $_SESSION['desconto'];
					echo guarani($totalGeral);
	?></td>
  </tr>
</table>
	...
	if ($_GET['acao'] == "insere") {
		...
		//if (isset ($_GET['id'])) {
			//if ($_GET['id']) {
			    ...
			    if ($reg_cont['n_prod'] > 0) {
					$sql_prod = "SELECT * FROM carrinho WHERE sessao = '".session_id()."' 
					AND controle = ".$_SESSION['controleCli2']." AND status = 'A' ";
				    $exe_prod = mysql_query($sql_prod, $base) or die (mysql_error());
				    $num_prod = mysql_num_rows($exe_prod);
					if ($num_prod > 0) {
					...
					if($_SESSION['controleCli2'] == 1 && $forma_pgto != 1 ){
					$mensagem = "Este Cliente Solamente al Contado";
					echo "<script language='javaScript'>alert('Este Cliente Solamente al Contado')</script>"; 
					exit();
					}
					...
					$sql_add = "INSERT INTO pedido
								(controle_cli, nome_cli, sessao_car, data_car, total_nota, situacao, 
								usuario_id, forma_pagamento_id, frete, vendedor, desconto)
								VALUES
								('".$reg_prod['controle']."', '".$_SESSION['nomeCli']."', '".$reg_prod['sessao']."', 
								NOW(), '$totalGeral', 'A', '$id_usuario','$forma_pgto', '$valor_frete', 
								'$vendedor',  '$desconto' )";
					...
					$sql_add = "INSERT INTO itens_pedido
								...
								VALUES
								($id_pedido,'".mysql_escape_string($reg_prodx['Codigo'])."', 
								'".mysql_escape_string($reg_prodx['descricao'])."', '".$reg_prodx['prvenda']."', 
								'".$reg_prodx['qtd_produto']."', '".$reg_prodx['sessao_car']."', '".$reg_prodx['codigo_prod']."')";
