<?
	...
	echo "<style>";
	echo  ".coluna{font-size:12px;  font-family:Verdana, Arial, Helvetica, sans-serif;}";
	echo  ".linha{background-color: #ECE9D8; font-size:12px; font-family:Verdana, Arial, Helvetica, sans-serif}";
	echo "</style>";
	...
	$sql_prod= "SELECT * FROM produtos WHERE codigo = ".$prod_id;
	...
	$conteudo.="<td>Custo</td>";
	$conteudo.="<td>%Margem A</td>";
	$conteudo.="<td>%Margem B</td>";
	...
	if($perfil_id <= 2){
	$conteudo.="<td class='coluna'>&nbsp;".($linha_prod['custo'])."</td>";
...
$conteudo = "<table width='100%' style='border: 1px solid #C6D9F1'>";
  $conteudo.="<tr>";
    $conteudo.="<td width='48%' height='58'><table width='97%' border='1'>";
      ...
      $sql = "SELECT ic.*, c.* FROM itens_pedido ic, pedido c  WHERE ic.referencia_prod = '".$linha_prod['Codigo']."' AND ic.id_pedido = c.id ORDER BY c.id DESC limit 0,07 ";
