...
dsRecCli = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: '../sistema/php/rec_cli_parcial.php',
			method: 'POST'
		}),   
			...
			items: [ 	            
            			...
            			handler: function(){ // fechar	
							...
							if(idPgr.length > 0){
								...
								maximizable: false,
								layout: 'fit',
								items: { html: "<iframe height='100%' width='100%' src='../sistema/pdf_recibo.php?idPgr="+idPgr+"' > </iframe>" },
								buttons: [
										{
										text: 'Cerrar',
										iconCls: 'icon-cancel',
...
formSintetico = new Ext.FormPanel({
	frame: true,
	closable: true,
	layout: 'form',
	items: [
		...
		iconCls: 'icon-search',
		handler: function(){ // fechar	
			geralSin = formSintetico.getForm().findField('geralSint').getValue();
			analitico(geralSin);
		}
		}
	]
...
function analitico(geralSin){
var geralSin = geralSin;
	...
	var win_rel_analitico = new Ext.Window({
		...
		maximizable: false,
		layout: 'fit',
		items: { html: "<iframe height='100%' width='100%' src='../sistema/relatorio_cli_analitico.php?opcao="+geralSin+"' > </iframe>" },
		buttons: [
			{
			text: 'Cerrar',
			iconCls: 'icon-cancel',
...
var dsContRec = new Ext.data.GroupingStore({
		proxy: new Ext.data.HttpProxy({
			url: '../sistema/php/contas_receber.php',
			method: 'POST'
		}),   
		reader:  new Ext.data.JsonReader({
			root: 'Clientes',
 ...
 var gridContRec = new Ext.grid.EditorGridPanel(
	    {
			...
			items: [ 	            
						...
						maximizable: false,
						layout: 'fit',
						items: { html: "<iframe height='100%' width='100%' src='../sistema/relatorio_cli.php?cli="+idCli+"' > </iframe>" },
						buttons: [
           						{
            					text: 'Cerrar',
								iconCls: 'icon-cancel',
			...
			*/
			{
						...
						iconCls: 'icon-users',
            			handler: function(){ // fechar	
							var winOpcaoGeral = new Ext.Window({
								title: 'Relatorio Sintetico',
								width: 300,
								height: 200,
								animCollapse: false,
...
///COMECA A GRID DAS FATURAS ///////////////////////////////////////////////
      ...
      proxy: new Ext.data.HttpProxy({method: 'POST', url: '../sistema/php/contas_receber.php'}),
      groupField:'nome',
...
gridContRec.on('rowdblclick', function(grid, row, e) {
	...
	BaixaConta = function(origem,ncredito,adevolver){
		if(origem != "NTCredito"){
