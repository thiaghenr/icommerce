...
saida_cheque = function(vlcompra,fatura,emissaofat){
  ...
  winSaiCheque = new Ext.Window({
				...
				handler: function(){
					...
					storeEntCheque.each(function( record ){
						...
						jsonSaidaCheque = Ext.encode(jsonData);
					winSaiCheque.close();
						/*
							 Ext.Ajax.request({
								url: 'php/saida_cheque.php',
					...
					handler: function(){ 
						Ext.MessageBox.confirm('Alerta', 'Deseja Cerrar Sin Facturar ?', function(btn) {
							if(btn == "yes"){
								...
								winSaiCheque.close();
								//dsCompraPgto.load();
								FormParcial.getForm().findField('formapagoconta').reset();
								dsCompraPgto.load(({params:{acao: 'ListaPgto', idcompranova: idcompranova,  'start':0, 'limit':5}}));
							}
							})	
					 }
