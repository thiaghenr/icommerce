...
saida_crediario = function(vlcompra){
vlcompra = vlcompra;
...
//console.info(vlcompra);
 ...
 storeEntCrediario = new Ext.data.GroupingStore({
			    proxy: new Ext.data.HttpProxy({
                url: 'php/saida_crediario.php',
				autoLoad: false,
                method: 'POST'
				}),
				groupField:'id',
 ...
 reader:  new Ext.data.JsonReader({
				...
				fields: [
						 ...
						 {name: 'totalCuentas', type: 'float'}
						 ]
			})					    
		});
		
	// define a custom summary function
     Ext.grid.GroupSummary.Calculations['totalCuentas'] = function(v, record, field){
...
var gridEntCrediario = new Ext.grid.GridPanel({
		plugins: [summary,editor],
		store: storeEntCrediario,
		height      : 500,
		tbar: [{
				text: 'Adicionar Cuenta',
				iconCls: 'icon-add',
				handler: function(){
				editor.newRecord();
				}
	...
	columns: [
		...
		{ header: '<b>Vencimiento</b>', dataIndex: 'data_validade', width: 215, name: 'data_validade',
		...
		},		
			{
				...
				id:'totalCuentas',
                dataIndex: 'totalCuentas',
                summaryType:'totalCuentas',
				fixed:true,
...
var FormEntCrediario = new Ext.FormPanel({
			frame: true,
			id: 'FormReq',
			closable: true,
			bodyStyle:'background-color:#CDB5CD',
			width       : '100%',
			items: [gridEntCrediario]
	});
  winEntCrediario = new Ext.Window({
				title: 'Catastro de Cuentas por Pagar',
				width:400,
				height:280,
				//autoWidth: true,
				//autoHeight: true,
				...
				handler: function(){
					...
					storeEntCrediario.each(function( record ){
						// Recebendo os dados
						jsonData.push( record.data );
					});
						jsonData = Ext.encode(jsonData);
							 Ext.Ajax.request({
								url: 'php/saida_crediario_desp.php',
								method: 'POST',
								remoteSort: true,
								params: {
								acao: 'CadContPagar', 
								...
								callback: function (options, success, response) {
										if (success) { 
											...
											if(jsonData.contaspagar > 0){
											Ext.MessageBox.alert('OK', 'Cuentas Catastradas: '+ jsonData.contaspagar, function(btn){
												if(btn == "ok"){
													tipopgto = "crediario";
												//	dsCompras.reload();
												//	formCompras.getForm().items.each(function(itm){itm.setDisabled(true)});
													winEntCrediario.hide();
													dsCompraPgto.load(({params:{acao: 'ListaPgto',   'start':0, 'limit':5}}));
