...
define('FPDF_FONTPATH','fpdf16/font/');
		...
		$sql_listaw = "SELECT p.nome,p.endereco,p.telefone,p.celular, c.nomecidade, r.* FROM requisicao r, proveedor p
		...
		$reg_listaw = mysql_fetch_array($exe_listaw, MYSQL_ASSOC);
			...
			$cotacao = $reg_listaw['id'];
			$codigo_cli = $reg_listaw['controle_cli'];
			$endereco = $reg_listaw['endereco'];
			$telefone = $reg_listaw['telefone'];
			$fax = $reg_listaw['fax'];
			$celular = $reg_contato['celular'];
			$ruc = $reg_listaw['ruc'];
			$email = $reg_contato['emailcontato'];
			$nomecidade = $reg_contato['nomecidade'];
...
$pdf->Image('LogoIndustec.jpg',30,14,35,12,jpg);
$pdf->Ln(5);
$pdf->Cell(80,0,"   ",0,1,'R');
$pdf->Ln(4);
$pdf->SetFont('arial','',6);
...
$pdf->Ln(4);
$pdf->Cell(66,0,"Tel./Fax (047) ******* / *******",0,1,'R');
$pdf->Ln(4);
$pdf->Cell(68,0,"Celular (0973) ******* / (0983) *******",0,1,'R');
$pdf->Ln(4);
$pdf->SetFont('arial','',9);
$pdf->Cell(55,0,"Katuete - Paraguay",0,1,'R');
...
//SQL PARA ITENS DO PEDIDO
  //  $sql_itens_pedido = "";
   ...
   $sql_itens_pedido = "SELECT * FROM itens_requisicao WHERE idrequisicao = '2' ";
      ...
      $num_itens_pedido = mysql_num_rows($exe_itens_pedido);
		...
		$sql_vend = "SELECT id_usuario,nome_user FROM usuario WHERE id_usuario = '".$_SESSION['vendedor']."' ";
		$exe_vend = mysql_query($sql_vend);
		$reg_vend = mysql_fetch_array($exe_vend, MYSQL_ASSOC);
		$_SESSION['nome_vendedor'] = $reg_vend['nome_user'];
...
$pdf->Cell(16,6,$reg_itens_pedido['qtd_produto'],0,0,'L');
$pdf->Cell(22,6,substr($reg_itens_pedido['Codigo'],0,10),0,0,'L');
$pdf->Cell(95,6,$reg_itens_pedido['descricao_prod']." ".$mat." ".$cor." ".$per." ".$aba." ".$reg,0,0,'L');
