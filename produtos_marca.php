...
define('FPDF_FONTPATH','fpdf17/font/');
 	...
 	$sql = "SELECT * FROM marcas WHERE id = '".$_GET['marca']."' ";
    $rs = mysql_query($sql) or die (mysql_error());
	$reg_lista = mysql_fetch_array($rs, MYSQL_ASSOC); 
	
	$_SESSION['nome_marca'] = $reg_lista['nom_marca'];
...
$this->Cell(13,5,"Marca:",0,0,'L');
$this->Cell(0,5,$_SESSION['nome_marca'],0,1,'L');
$this->Ln(8);
$this->SetY("30");
$this->SetX("20");
//$this->Cell(172,0,'',1,1,'L');
...
//$pdf->Ln(1);
    ...
    $sql = "SELECT m.*, p.id,Codigo,Descricao,Estoque,marca,valor_a 
	FROM produtos p, marcas m WHERE p.marca = m.id AND m.id = '".$_SESSION['marca']."' ORDER BY p.Codigo ASC ";
    $rs = mysql_query($sql) or die (mysql_error());
