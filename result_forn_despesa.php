<?
	...
	$forn_cod = $_POST['CodigoForn'];
  
    $sql = "SELECT * FROM proveedor WHERE id LIKE '%$cod%' AND nome LIKE '%$nom%'  ORDER BY Nome asc limit 0,40";
...
<html>
<? echo 'Fornecedores encontrados = '.$num_cli; ?>
<div align="center" class="Estilo1">
<table width="49%" border="0">
  <tr>
    <td width="15%" bgcolor="#D4D0C8">Nombre:</td>
    <td width="85%">
	<form name="nome" method="POST" action="result_forn_despesa.php">
	<input type="text" size="40" name="nom" value="<?=$_SESSION['nomeForn']?>"></td>
  </tr></form>
</table>
  <table width="100%" border="0">
    <tr>
      <td bgcolor="#FF0000"><div align="center">
          <p class="Estilo3 Estilo2 Estilo4">FORNECEDORES</p>
      </div></td>
    </tr>
  </table>
</div>
...
function navegacao(lugar){
 if(lugar=='voltar'){
  ...
  window.location.href='lancar_despesa.php';
 }
 else if(lugar=='Curso'){
...
*/
	obj.onKeyEnter = function(event){
        ...
        url = "lancar_despesa.php?acao=add&";
		var query = "CodigoForn=" + this.getCellText(0, row) + "&NomeForn=" + this.getCellText(1, row) ;
		url+=query;
        window.location = url;
