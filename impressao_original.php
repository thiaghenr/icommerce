<?
   ...
   // $pedido_id = 46;
     ...
     function createCabecalhoPedido($pedido_id){
        //SQL PARA PEDIDO
        $sql_pedido = "SELECT p.*, c.* FROM pedido p, entidades c WHERE p.id = '$pedido_id' AND p.controle_cli = c.controle ";
...
$cabecalho_nota = "
 ...
 return $cabecalho_nota;
    ...
    function createCorpoItensPedido($pedido_id,&$inicio,$limitNotas,$totalValorNota){
      ...
      $sql_itens_pedido = "SELECT p.forma_pagamento_id,p.desconto, it.* FROM itens_pedido it, pedido p WHERE it.id_pedido = '$pedido_id' AND p.id = it.id_pedido LIMIT $inicio,$limitNotas";
...
$rodape_nota = "
 ===============================================================================
    ...
    *** CONTROL INTERNO *** ".str_pad("Descuento: GR$",41," ",STR_PAD_LEFT)." ".str_pad(number_format("$desconto",0,",","."),9," ",STR_PAD_LEFT)."
                 ".str_pad("TOTAL: GR$",30," ",STR_PAD_LEFT)." ".str_pad(number_format("$total",0,",","."),9," ",STR_PAD_LEFT)."
