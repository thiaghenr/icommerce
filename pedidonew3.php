...
FinPedido = function(){
	...
	jsonData = "[";
		for(d=0;d<dsProds.getCount();d++) {
		record = dsProds.getAt(d);
	...
	//alert(jsonData);
		 ...
		 Ext.Ajax.request({
			    			...
			    			clienteIns: controle,
							usuario_id:  formFinalisar.getForm().findField('usuario_id').getValue(),
										...
										if (success) { 
											...
											var jsonData = Ext.util.JSON.decode(response.responseText);
												if(jsonData.del_count){
													...
													window.open('pedido_ok.php?id_pedido='+jsonData.Pedido +'','popup','width=750,height=500,scrolling=auto,top=0,left=0');
 		...
 		dsProdPedido = new Ext.data.Store({
 	     ...
 	     var grid_listaProdDetalhes = new Ext.grid.GridPanel({
			...
			keypress: function(e){
						...
						var Record = Ext.data.Record.create(['idprod','codigo', 'grupo','descricao','qtd_produto','prvenda','totals']);
...
var gridProd = new Ext.grid.EditorGridPanel({
   ...
   loadMask: {msg: 'Carregando...'},
     columns: [
	  ...
	  {id: 'codigo', header: 'Codigo', summaryType: 'count', width: 200, dataIndex: 'codigo' },
      ...
      {id: 'descricao',  header: 'Descripcion', width: 300, dataIndex: 'descricao'},
