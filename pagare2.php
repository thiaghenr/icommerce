...
define('FPDF_FONTPATH','fpdf/font/');
		...
		$sql_listaw = "SELECT * FROM venda WHERE id = '".$_GET['venda']."' ";
...
$sql_lista = "SELECT v.id AS ide, cr.*, cl.endereco, cl.telefonecom,nome,ruc,endereco FROM venda v, contas_receber cr, clientes cl WHERE v.id = '".$_GET['venda']."' AND v.controle_cli = cl.controle AND cr.id = '".$_GET['pagare']."' ";
	...
	//if ($num_lista > 0) {
		//$total_carrinho = 0;
			...
			$diab = substr($data2,8,2) ; //. "/" .substr($data2,5,2) . "/" . substr($data2,0,4);
			$mesb = substr($data2,5,2) ;
			$anob = substr($data2,0,4) ;
...
$pdf->Cell(13,5,$reg_lista['id'],0,0,'R');
$pdf->Cell(2,5,"/",0,0,'R');
$pdf->Cell(3,5,$reg_lista['nm_parcela'],0,0,'L');
$pdf->Cell(146,5,"G$.:",0,0,'R');
$pdf->Cell(24,5,guarani($reg_lista['vl_parcela']),0,0,'R');
$pdf->Ln(7);
$pdf->Cell(27,5,"Vencimento  en ",0,0,'L');
$pdf->Cell(6,5,$dia,0,0,'L');
$pdf->Cell(8,5," de ",0,0,'L');
$pdf->Cell(26,5,$mes,0,0,'L');
$pdf->Cell(12,5,$ano,0,0,'L');
...
$pdf->Cell(0,5,extensodolar($reg_lista['vl_parcela']),0,0,'L');
$pdf->Ln(3);
$pdf->SetFont('arial','',8,'');
$pdf->Ln(4);
...
$pdf->Ln(3);
$pdf->SetFont('arial','',9,'');
$pdf->Cell(50,5,"Pagadero en CIUDAD DEL ESTE, ",0,0,'L');
$pdf->Cell(6,5,$diab,0,0,'L');
$pdf->Cell(8,5," de ",0,0,'L');
$pdf->Cell(26,5,$mesb,0,0,'L');
$pdf->Cell(12,3,$anob,0,1,'L');
...
$pdf->Cell(0,5,$reg_lista['telefonecom'],0,0,'L');
