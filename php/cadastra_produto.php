...
if($acao == 'Cadastra'){
		...
		$sql_per = "INSERT INTO produtos (Codigo, Descricao, Descricaoes, custo, part_number,  serial, 
					...
					margen_a, margen_b, margen_c, valor_a, valor_b, Estoque, qtd_min, iva, 
					qtd_bloq, local, embalagem, user, data, marca, grupo, cod_original2, custo_medio, 
					custo_anterior, imagem, obs, peso, ref) 
		...
		UCASE('$Codigo_Fabricante3'), '$pr_min', '$margen_a', '$margen_b', '$margen_c', '$valor_a', '$valor_b', 
		'$Estoque', '$qtd_min', '$iva', '$qtd_bloq', UCASE('$local'), UCASE('$embalagem'), UCASE('$user'), NOW(), 
		'$marca', '$grupo', UCASE('$cod_original2'), '$custo', '$custo', '$filename', '$obsprod', '$peso', '$ref' )";
...
if($acao == 'Update'){
	...
	$sql_per = "UPDATE produtos SET Codigo=UCASE('$Codigo'), Descricao=UCASE('$Descricao'),  Descricaoes=UCASE('$Descricaoes'), 
	custo='$custo',  marca=UCASE('$marca'), grupo='$grupo', cod_original=UCASE('$cod_original'), 
	Codigo_Fabricante=UCASE('$Codigo_Fabricante'), Codigo_Fabricante2=UCASE('$Codigo_Fabricante2'), 
	Codigo_Fabricante3=UCASE('$Codigo_Fabricante3'), pr_min='$pr_min', margen_a='$margen_a', margen_b='$margen_b', 
	valor_a='$valor_a', valor_b='$valor_b', qtd_min='$qtd_min', iva='$iva',  embalagem=UCASE('$embalagem'), data=NOW(), 
	user=UCASE('$nome_user'), local=UCASE('$local'), marca='$marca', grupo='$grupo', cod_original2=UCASE('$cod_original2'), 
	imagem='$filename', peso='$peso', ref='$ref' WHERE id = '".$idProd."' ";
$exe_per = mysql_query($sql_per, $base) or die (mysql_error().'-'.$sql_per);
