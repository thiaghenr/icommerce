...
if($acao == 'Faturar'){
	 ...
	 $caixa_id = $reg_caixa_balcao['id'];
      ...
      if(!	($caixa_id)){
          echo "{success: true,response: 'No ha caja abierto para este usuario'}";
          exit();
      }
   ...
   $tipo_venda = $idForma;
    ...
    if ($num_prodp > 0) {
		...
		$valorcrd = $valorcrd - $valor_abatido;
            ...
            $sql_lancamento_caixa_balcao.= "VALUES ('3','$caixa_id',now(),'$vl_abat', 'Credito p/ $nomecli')";
        ...
        $sql_addv = "INSERT INTO venda
    	    (data_venda, pedido_id, num_boleta, valor_venda, imposto_id, controle_cli, st_venda, entrada, desconto, vlcredito )
    		VALUES
    		(NOW(), '".$reg_prodp['id']."', '0', '".$reg_prodp['total_nota']."',  '10', '".$reg_prodp['controle_cli']."','".$st_venda."',
			'".$reg_prodp['entrada']."', '".$reg_prodp['desconto']."', '$vl_abat' )";
...
$sql_prodp = "SELECT total_nota,desconto FROM pedido WHERE id =  '$id_pedido' ";
	...
	$vl_total_nota =  $vl_total_nota - $vl_abat;
...
//echo    $entidade."entidade ";	
   ...
   function lacamentoContaReceber($vl_total_nota,$vl_entrada,$forma_pgto_id,$entidade, $id_venda, $user, $vl_abat){
	  ...
	  $sql = mysql_query("SELECT valor_venda,desconto,vlcredito FROM venda WHERE id = '$id_venda' ");
	  $reg = mysql_fetch_array($sql);
	  $desconto = $reg['desconto'];
	  $vl_total_nota = $reg['valor_venda'];
	  $vl_abat = $reg['vlcredito'];
    ...
    if ($tipo_venda == '1') {
        ...
        $vl_entrada = str_replace(',', '.',$_POST['valor_entrada']);
            ...
            lacamentoContaReceber($vl_total_nota,$vl_entrada,$forma_pgto_id,$entidade,$id_venda, $user, $vl_abat);
