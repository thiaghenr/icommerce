...
class Table_Cell_Frame_Reflower extends Block_Frame_Reflower {
  ...
  function reflow() {
    ...
    $w = 0;
    foreach ((array) $cells["columns"] as $i ) {  //cms  foreach ( $cells["columns"] as $i ) {
    ...
    // cms  $cell_height = $height / count($cells["rows"])  + $top_space + $bottom_space;
	$cell_height = $height / (count($cells["rows"])?count($cells["rows"]):1) + $top_space + $bottom_space; 
    foreach ((array)$cells["rows"] as $i) //cms foreach ($cells["rows"] as $i)
