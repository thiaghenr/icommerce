...
$z=1;
		while($z <= $qtd) {
			...
			$sql_per = "INSERT INTO lanc_contas (receita_id, plano_id, documento, dt_lanc_desp, venc_desp, dt_fatura_desp, 
						desc_desp, valor, usuario_id, nm_total_parcela, nm_parcela, entidade_id, valor_total, plan_codigo ) 
						VALUES('$receita', '$idnode', UCASE('$ndoc'), NOW(), NOW(), '$dt_fatura',  UCASE('$obs'), 
						'$valor', '$user', '1', '1', '$fornecedor_id', '$valor', '$nomedesp')";
...
if(isset($_POST['ckbVista'])){
		...
		$sql_caixa = "INSERT INTO lancamento_caixa (receita_id, caixa_id, data_lancamento, num_nota, despesa_cod, contas_pagar_id, 
					valor, fornecedor_id, lanc_despesa_id, contaid, historico) 
					VALUES( '$receita', '$caixa_id', NOW(), UCASE('$ndoc'), '$nomedesp', '$cont_pag_id', '$valor', '$fornecedor_id', '$lanc_desp_id', '$idnode', '$obs') ";
