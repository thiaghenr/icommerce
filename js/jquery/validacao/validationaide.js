...
jQuery.validationAide = {
	getDefaultValidationRules : function(){
		...
		rules.add('valida_email', 'Por favor insira um e-mail válido. Exemplo nome@dominio.com', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return true;
			return /\w{1,}[@][\w\-]{1,}([.]([\w\-]{1,})){1,3}$/.test(fieldValue);
		});
		
		rules.add('valida_numero_ponto', 'Por favor insira somente números e pontos.', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return true;
			return (!isNaN(fieldValue) && !/^\s+$/.test(fieldValue));
		});
		
		rules.add('valida_numero', 'Por favor insira somente números.', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return true;
		...
		rules.add('valida_texto_numero', 'Por favor use somente letras de (a-z) ou números de (0-9).', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return true;
		...
		rules.add('valida_url', 'Por favor insira uma URL válida.', function(fieldValue, fieldObj) {
			if (fieldValue == "")	return true;
	...
	resetForm : function (formId, options){
		...
		if (typeof jQuery.fn.fieldStringVal != "function"){
			alert("Atenção, a validação só  vai funcionar se stringaide.js for carregado!");
		}
