...
$sql = "SELECT * FROM produtos WHERE ((Descricao LIKE '$desc%') AND (Codigo LIKE '%$ref%')) OR ((cod_original2 LIKE '%$ref%') AND (Descricao LIKE '$desc%') ) OR ((cod_original LIKE '%$ref%') AND (Descricao LIKE '$desc%')) OR ((Codigo_Fabricante LIKE '%$ref%') AND (Descricao LIKE '$desc%')) ORDER BY CAST(Codigo AS UNSIGNED) asc limit 100  ";    $rs = mysql_query($sql) or die (mysql_error());
	
	$num_prod = mysql_num_rows($rs);
	
    $str_dados="[";
   while($linha=mysql_fetch_array($rs)){
	//$novovalor = $linha['Preco_Venda'] + 2 ;
		...
		$str_dados.="'".addslashes($linha['valor_a'])."',";
		$str_dados.="'".addslashes($linha['valor_b'])."',";
		$str_dados.="'".addslashes($linha['valor_c'])."'],";
    }
...
<table width="49%" border="0">
  <tr>
	...
	<td width="85%">
	<form name="Descricao" method="POST" action="res_pes_prodc.php">
		...
		Ext.onReady(function(){
		    ...
		    var grid = new Ext.grid.GridPanel({
		        ...
		        columns: [
						...
						{header: "Preco", width: 80, sortable: true, dataIndex: 'Preco'},
