...
Ext.get("cad_planContas").on('click',function(s,e){
    Ext.QuickTips.init();
...
if (NovoPlan == null){
 ...
 if (cad_plan_contas == null){
				cad_plan_contas = new Ext.Window({
					id:'cad_plan_contas'
					, title: "CP SA"
	                , layout: 'form'
	                , width: '55%'
	                ...
	                , plain: true
					, modal: false
					, items:[FormPlanContas]
					,buttons: [
           						{
            					text: 'Cerrar',
            					handler: function(){ // fechar	
     	    					cad_plan_contas.destroy();
								NovoPlan.destroy();
  			 					}
        			}
					]					
